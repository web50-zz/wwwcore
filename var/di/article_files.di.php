...
* @package	SBIN Diesel
*/
class di_article_files extends data_interface
{
	...
	*/
	protected $name = 'article_files';

	/**
	* @var	string	$path_to_storage	Путь к хранилищу файлов каталога
	*/
	public $path_to_storage = 'article_files/';
	...
	*/
	...
	protected function sys_list()
	{
		...
		$this->extjs_grid_json(array('id', 'order', 'title', 'file_type', 'real_name', '"'.$this->get_url().'"' => 'url'));
	}
	...
	*/
	protected function sys_reorder()
	{
		...
		if ($opos < $npos)
			$query = "UPDATE `{$this->name}` SET `order` = IF(`id` = :id, :npos, `order` - 1) WHERE `order` >= :opos AND `order` <= :npos AND `article_id` = :cid";
		else
			$query = "UPDATE `{$this->name}` SET `order` = IF(`id` = :id, :npos, `order` + 1) WHERE `order` >= :npos AND `order` <= :opos AND `article_id` = :cid";

		$this->_flush();
		$this->connector->exec($query, $values);
		$this->fire_event('onSet', array(null, array('article_id' => $values['cid'])));
	...
	*/
	protected function sys_set()
	{
		...
		if ($file !== false)
		{
			...
			// Если мы имеем имя файла и тип файла изобаржение, то создаём превьюшки
			if ($file['real_name'] && $this->get_args('file_type') == 1)
			{
				...
				{
					file_system::remove_file("thumb-{$old_file_name}", $this->get_path_to_storage());
				}

				// Формируем два превью для изображения
				$width = registry::get('article_gallery_image_width');
				$height = registry::get('article_gallery_image_height');
				if(!($width>0)){
					$width = 200;
				}
				if(!($height)>0){
				...
				$this->create_preview($file, 'thumb-', $width, $height, true, null);
