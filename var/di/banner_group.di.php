<?php
/**
*	Интерфейс данных "Группы баннеров"
*
* @author	Anthon S. Litvinenko <a.litvinenko@web50.ru>
* @package	SBIN Diesel
*/
class di_banner_group extends data_interface
{
	public $title = 'Группы баннеров';
	...
	*/
	protected function sys_available()
	{
		...
		$this->connector->exec('SELECT `bg`.`id`, CONCAT(`bg`.`title`, " [", `bg`.`width`, "x", `bg`.`height`, "]") AS `title`, COUNT(*) AS `count`
			FROM `banner` AS `b` LEFT JOIN `banner_group` AS `bg` ON `b`.`banner_group_id` = `bg`.`id`
	...
	*/
	protected function unset_recursively($id)
	{
		...
		$sc = data_interface::get_instance('banner');
		$sc->remove_files($ids);
	}
}
?>
