ui.article_files.main = Ext.extend(ui.article_files.grid, {
	Load: function(data){
		this.setParams({}, true);
	},
	Add: function(){
		...
		app.on({
			apploaded: function(){
				var f = new ui.article_files.item_form();
				...
				var wid = this.getKey();
				w.show(null, function(){f.Load({article_id: wid})});
			},
			apperror: showError,
			scope: this
		});
		app.Load('article_files', 'item_form');
	},
	Edit: function(){
		var app = new App({waitMsg: 'Загрузка формы'});
		app.on({
	...
	constructor: function(config)
	{
		...
		Ext.apply(this, {
			...
			tbar: [
				...
				'->', {iconCls: 'help', handler: function(){showHelp('article_files')}}
			]
		});
		Ext.apply(this, {
			listeners: {
				...
				rowmove: function(target, row){
					Ext.Ajax.request({
						url: 'di/article_files/reorder.do',
						method: 'post',
		...
		ui.article_files.main.superclass.constructor.call(this, config);
	},
