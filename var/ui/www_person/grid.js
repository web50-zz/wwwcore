ui.www_person.grid = Ext.extend(Ext.grid.EditorGridPanel, {
	...
	constructor: function(config){
		Ext.apply(this, {
			store: new Ext.data.Store({
				proxy: new Ext.data.HttpProxy({
					api: {
						read: 'di/www_person/list.js',
						create: 'di/www_person/set.js',
						update: 'di/www_person/mset.js',
						destroy: 'di/www_person/unset.js'
					}
				}),
		...
		Ext.apply(this, {
			...
			sm: new Ext.grid.RowSelectionModel({  
				listeners: {
					...
					beforerowselect: function(sm, i, ke, row){
						this.ddText = drag(row.data.preview, row.data.person_name);
					},
					scope: this
				}  
			}),
			colModel: new Ext.grid.ColumnModel({
				...
				columns: [
					{header: 'Фото', dataIndex: 'preview', xtype: 'templatecolumn', tpl: image},
					{header: 'Имя', dataIndex: 'person_name'},
					{header: 'Должность', dataIndex: 'person_title', id: 'expand'}
				]
			}),
		...
		ui.www_person.grid.superclass.constructor.call(this, config);
		this.init(config);
	},
